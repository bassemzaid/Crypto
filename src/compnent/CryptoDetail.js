...
const CryptoDetail = () => {
  ...
  const [state, setState] = useState("3h");
  const { data, isFetching } = useGetDetailQuery(coinId);
  const { data: coinHistory } = useGetHistoryQuery({ coinId, state });
  ...
  return (
    <Col className="coin-detail-container">
      <Col className="coin-heading-container">
        ...
        <p>{cryptoDetails.name} Live price right now !</p>
      </Col>
      <Select
        showSearch
        defaultValue={state}
